...
from runtime_manager import child_environment, find_entrypoint, prepare_command
...
async def safe_edit_text(message: types.Message, text: str, **kwargs):
    """Edit message text, ignore 'message is not modified' errors (common on repeated clicks)."""
    try:
...
async def safe_edit_reply_markup(message: types.Message, **kwargs):
    """Edit reply markup, ignore 'message is not modified' errors."""
    try:
...
if not OWNER_ID_STR or not ADMIN_ID_STR:
    logger.warning("OWNER_ID/ADMIN_ID not set. Admin features will be disabled until you set them.")
else:
    try:
        OWNER_ID = int(OWNER_ID_STR)
        ADMIN_ID = int(ADMIN_ID_STR)
    except ValueError:
        logger.warning("OWNER_ID/ADMIN_ID are invalid. Admin features disabled until you set valid integers.")

YOUR_USERNAME = YOUR_USERNAME or '@theonlyziad'
UPDATE_CHANNEL = UPDATE_CHANNEL or 'https://t.me/zeedtek'
...
def safe_user_path(user_folder: Path, relative_name: str) -> Path:
...
# =========================
# FORCE JOIN (Back4App safe)
# =========================

FORCE_CHANNEL = os.getenv("FORCE_CHANNEL", "@zeedtek")

def join_keyboard():
    ch = FORCE_CHANNEL.replace("@", "")
...
async def is_joined(bot_obj: Bot, user_id: int) -> bool:
    try:
        member = await bot_obj.get_chat_member(FORCE_CHANNEL, user_id)
...
class ForceJoinMiddleware(BaseMiddleware):
    async def __call__(self, handler, event, data):
        ...
        bot_obj = data.get("bot")  # aiogram injects bot here
        if user is None or bot_obj is None:
            return await handler(event, data)

        uid = user.id
        if uid in {OWNER_ID, ADMIN_ID} and uid != 0:
            return await handler(event, data)

        ok = await is_joined(bot_obj, uid)
        if ok:
            return await handler(event, data)
        ...
        if isinstance(event, types.Message):
            await event.answer(
                "⚠️ لازم تدخل القناة باش تستعمل البوت.\n\n📌 Channel: " + FORCE_CHANNEL,
                reply_markup=join_keyboard()
            )
        elif isinstance(event, types.CallbackQuery):
            try:
                await event.answer("⚠️ لازم تدخل القناة أولاً.", show_alert=True)
            except Exception:
                pass
            if event.message:
...
async def check_join_cb(call: types.CallbackQuery):
    ok = await is_joined(bot, call.from_user.id)
    if ok:
        await call.answer("✅ تم التحقق", show_alert=True)
    else:
        await call.answer("❌ مازلت مادخلتش", show_alert=True)
...
"""
    await safe_edit_text(callback.message, welcome_text, reply_markup=get_main_keyboard(user_id), parse_mode="HTML")
...
"""
    ...
    await safe_edit_text(callback.message, upload_text, reply_markup=back_keyboard, parse_mode="HTML")
    await callback.answer()
...
"""
    ...
    await safe_edit_text(callback.message, text, reply_markup=get_admin_panel_keyboard(), parse_mode="HTML")
    await callback.answer()
...
async def handle_document(message: types.Message):
    ...
    file_name = Path(document.file_name or "upload.bin").name
    ...
    if document.file_size and document.file_size > MAX_UPLOAD_BYTES:
        await message.answer(f"❌ File is too large. Maximum: {MAX_UPLOAD_BYTES // (1024 * 1024)} MB")
        return
...
async def callback_run_script(callback: types.CallbackQuery):
    ...
    try:
        ...
        process = subprocess.Popen(
            ...
            env=child_environment(entrypoint),
        )
...
async def callback_stop_script(callback: types.CallbackQuery):
    ...
    try:
        ...
        if callback.from_user.id in admin_ids:
            await safe_edit_text(callback.message, "🛑 Script stopped!", parse_mode="HTML")
        else:
...
async def callback_extract_zip(callback: types.CallbackQuery):
    ...
    user_folder = UPLOAD_BOTS_DIR / str(user_id)
    try:
        zip_path = safe_user_path(user_folder, file_name)
    except ValueError:
        await callback.answer("❌ Invalid ZIP path!", show_alert=True)
        return
...
Please wait...
"""
        await safe_edit_text(callback.message, status_text, parse_mode="HTML")
        ...
        with zipfile.ZipFile(zip_path, 'r') as zip_ref:
            ...
            for info in infos:
                if info.is_dir():
                    continue
                target = safe_user_path(user_folder, info.filename)
...
"""
        ...
        await safe_edit_text(callback.message, success_text, reply_markup=keyboard, parse_mode="HTML")
